...
module.exports = {
    '/gen/lib.js': async function() {
        const { outputFiles } = await bundleJs(path.join(discoveryDir, 'src/lib.js'));
        const { contents } = outputFiles[0];

        return new TextDecoder('utf-8').decode(contents);
    },
    '/gen/lib.css': async function() {
        const { content } = await bundleCss(path.join(discoveryDir, 'src/lib.css'));

...
const { runScript, getDataPlaceholder, getModePlaceholder, getSetupModelPlaceholder } = require('./utils');
...
module.exports = {
    '/gen/setup.js': function(modelConfig, options, config, data = 'data.json') {
        ...
        return Promise.resolve('export default {' + Object.entries(setup).map(([key, origValue]) => {
            ...
            if (key === 'mode') {
                value = `function ${getModePlaceholder()}(){return${value}}()`;
            }

            if (key === 'model') {
                value = `function ${getSetupModelPlaceholder()}(){return${value}}()`;
            }

            if (key === 'data') {
                value = `JSON.parse(function ${getDataPlaceholder()}(){return${options.singleFile ? ' null' : JSON.stringify(value)}}())`;
            }

...
function prepareModel({ name, slug, cache, meta }) {
    return {
        name,
        slug,
        cache: Boolean(cache),
...
module.exports = {
    '/gen/setup.js': function(modelConfig, options, config, data = 'data.json', dataPlaceholder) {
        let setup = {
            name: config.name,
            mode: config.mode,
        ...
        return Promise.resolve('export default {' + Object.entries(setup).map(([key, origValue]) => {
            ...
            if (key === 'data') {
                value = `JSON.parse(${options.singleFile ? dataPlaceholder : JSON.stringify(value)})`;
            }
    ...
    '/gen/build.html.zip': (modelConfig, options) => {
        const { slug } = modelConfig;
        const args = [];

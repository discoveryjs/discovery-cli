...
function normalizeConfig(config, model, basedir) {
    ...
    result.upload = 'upload' in result ? result.upload : false;
    result.embed = 'embed' in result ? Boolean(result.embed) : false;
    ...
    result.models = Object.keys(result.models).reduce((res, slug) => {
        if (!model || model === slug) {
            ...
            modelConfig.download = getDownloadUrl(
                'download' in modelConfig
                    ? Boolean(modelConfig.download)
                    : result.download
            );

            modelConfig.upload = 'upload' in modelConfig
                ? modelConfig.upload
                : result.upload;

            modelConfig.embed = 'embed' in modelConfig

...
function normalize(config, options) {
    ...
    if (!config.models) {
        ...
        result = {
            name: 'Discovery',
            mode: undefined, // to preserve key position in the config
            ...config,
    ...
    result.models = Object.keys(result.models).reduce((res, slug) => {
        if (!model || model === slug) {
            const modelConfig = {
                slug, // for a position
                ...modelBaseConfig,
                ...normalizeModelConfig(...resolveModelConfig(result.models[slug], basedir)),
                slug  // for a value
            };
            ...
            modelConfig.cache = modelConfig.cache
                ? '.discoveryjs.[slug].[timestamp].[hash].cache'
                : false;

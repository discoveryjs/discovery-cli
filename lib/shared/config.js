...
function normalizeModelConfig(config, basedir) {
    ...
    if (typeof normalizedConfig.data === 'string') {
        normalizedConfig.data = path.resolve(basedir, normalizedConfig.data);
    }

    normalizedConfig.view = { ...normalizedConfig.view };
    normalizedConfig.view.basedir = path.resolve(basedir, normalizedConfig.view.basedir || '');
    normalizedConfig.view.assets = (Array.isArray(normalizedConfig.view.assets) ? normalizedConfig.view.assets : [])
        .map(p => path.resolve(normalizedConfig.view.basedir, p));
    ...
    result.models = Object.keys(models).reduce((res, slug) => {
        if (!model || model === slug) {
            ...
            if (modelConfig.view.libs) {
                console.error(`[${slug}] model.view.libs is not supported anymore, use require() or ES6 import expressions instead`);
            }

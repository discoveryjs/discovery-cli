...
module.exports = async function(config, options, esbuildConfig, { cacheDispatcher, filter } = {}) {
    ...
    for (const modelConfig of config.models) {
        ...
        if (modelConfig.script) {
            ...
            scriptEntryPoints.push(`${slug}/model-script.js`);
        }
    }
    ...
    const scripts = await esbuild.build({
        ...
        format: 'esm',
        entryPoints: typeof filter === 'function'
            ? scriptEntryPoints.filter(ref => filter(ref))
            : scriptEntryPoints
    });

    const result = {
        warnings: [...assets.warnings, ...scripts.warnings],
        errors: [...assets.errors, ...scripts.errors],
        outputFiles: [...assets.outputFiles, ...scripts.outputFiles]
    };

...
function createCacheDispatcher(models, options) {
    ...
    const cacheFilenameTemplate = path.resolve(cachedir, '.discoveryjs.[slug].[timestamp].[hash].cache');
    ...
    const cleanupObsoleteCaches = () => {
        ...
        cleanupObsoleteTmpCaches();
    };

    // clean up *.tmp files
    const cleanupObsoleteTmpCaches = () => {
    ...
    return {
        ...
        genModelCacheFilename(slug) {
            return genModelCacheFilename(cacheBySlug.get(slug) || { slug, file: cacheFilenameTemplate });
        },
        genModelCacheTempFilename(slug) {
            return genModelCacheFilename(cacheBySlug.get(slug) || { slug, file: cacheFilenameTemplate }) + TMP_CACHE_EXTNAME;
        },

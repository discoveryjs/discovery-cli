...
function genCacheFilename(modelConfig) {
    ...
    const dict = new Map([
        ['slug', slug],
        ['timestamp', Date.now()],
        ['hash', modelConfig.cachePersistent ? getDataHash(modelConfig) : '']
    ]);

    return cache && cache.replace(/\[(\S+?)\]\./g, (m, key) => dict.get(key) ? dict.get(key) + '.' : '');
}
    ...
    for (const file of fs.readdirSync(cachedir)) {
        ...
        if (modelConfig) {
            ...
            if (all) {
                ...
                modelConfig.cacheTtl > cacheAge
            ) {
                if (modelConfig.cachePersistent && cacheInfo.hash === getDataHash(modelConfig)) {
                    cachesByModel[slug].push(cacheInfo);
                } else {
                    fs.unlink(fullPath, (err) => {
                        ...
                        if (err) {
                            logSlugError(slug, `Delete invalid cache "${relFilename}" error:`, err);
                        } else {
                            logSlugMsg(slug, `Invalid cache deleted: ${relFilename}`);
                        }
                    });
                }
            }
...
function createCacheDispatcher(models, { configFile, cachedir, bgUpdate, prettyData }) {
    ...
    return {
        ...
        write: writeCache,
        warmup(slug) {
            return getActualCache(cacheBySlug.get(slug).model)
                ? Promise.resolve()

...
function getPrevModelCacheTimestamp(modelCache, cacheInfo) {
    ...
    switch (typeof ttl) {
        case 'string':
            return cron.parseExpression(ttl, { utc: true }).prev().getTime();
...
function getNextModelCacheTimestamp(modelCache, cacheInfo) {
    ...
    switch (typeof ttl) {
        case 'string':
            return cron.parseExpression(ttl, { utc: true }).next().getTime();
...
function getModelActualCache(modelCache) {
...
function createCacheDispatcher(models, options) {
    ...
    const scheduleBgUpdate = (modelCache, cacheInfo) => {
        ...
        logSlugMsg(slug, `${bgUpdateTimer ? 'Re-schedule' : 'Schedule'} background data cache update in ${

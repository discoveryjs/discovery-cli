...
const discoveryNs = /^@discoveryjs\/discovery\//;
const isolateRootPrefix = 'isolate-style-root:';

function resolvePath(ref, basepath) {
    if (discoveryNs.test(ref)) {
        basepath = discoveryDir;
        ref = ref.replace(discoveryNs, '');
    }

    return {
        basepath,
        ref,
        resolved: path.resolve(basepath, ref)
    };
}
...
module.exports = function(filename, options) {
    const { isolate, isolateRoots = [], modules } = options || {};

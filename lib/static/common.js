...
import { inflate } from './inflate/decoder.js';
...
export function load(module, styles, setup, dataLoaderOptions) {
    ...
    if (typeof origLoaderPush === 'function') {
        window.discoveryLoader.push = function(chunk, compressed) {
            origLoaderPush(compressed ? inflate(chunk) : chunk);
        };
    }
    ...
    return Promise.all([
        ...
        init({ ...setup, styles }, loadData.progressbar, loadData.disposeEmbed?.(), data, context)
    ).then(() => {
        loadData.el.remove();
    }, (error) => {
        ...
        if (!error.supressLoadDataError) {
            ...
            el.className = 'error';
            el.append(document.createTextNode('[ERROR] ' + error + (error.stack ? '\n\n' + error.stack : '')));

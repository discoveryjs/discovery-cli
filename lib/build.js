...
function makeBundles(pathResolver, modelConfig, options, config, jsFilename, jsBundleOptions, cssFilename, cssBundleOptions) {
    return utils.section('Build bundles', () => Promise.resolve()
        ...
        .then(() => {
            ...
            let genData = gen['/data.json'](modelConfig, options);
...
function createModel(pathResolver, modelConfig, options, config, jsBundleOptions, cssBundleOptions) {
    ...
    return Promise
        ...
        .then(() => !options.allowDownload && utils.section('Disabling report download option in bundles', () => {
            fs.writeFileSync(
                pathResolver('model.js'),
                fs.readFileSync(pathResolver('model.js'), 'utf8')
                    .replace(new RegExp(`function ${utils.MODE_PLACEHOLDER}\\(\\){return.*?}\\(\\)`), '"single"')
                    .replace(new RegExp(`function ${utils.SETUP_MODEL_PLACEHOLDER}\\(\\){return.*?}\\(\\)`), JSON.stringify({ ...config.models[0], download: false, cache: false}))
            );
        }))

...
function createSlugFilesMap(entryPoints, pattern, buildPattern) {
    const map = new Map();
    const addFile = (slug, filename, content) => map.get(slug).set(filename, {
    ...
    for (const file of buildResult.outputFiles) {
        ...
        if (slug) {
            if (/^model-script\.[cm]?[tj]s$/.test(filename)) {
                addModelScript(slug, filename, content);
            } else {
    ...
    await utils.section('Generate scripts', () => {
        for (const [slug, scripts] of modelScripts) {
            if (scripts.size === 0) {
                ...
                utils.section(slug, () => {
                    for (const { outputPath, content } of scripts.values()) {
                        utils.println(`${outputPath} (${(content.byteLength / 1000).toFixed(1)}kB)`);
                        outputFiles.set(outputPath, content, slug);
                    }
                });
            }

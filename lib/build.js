    ...
    return Promise
        ...
        .then(() =>
            Promise.all([
                '/model-index.html',
                '/gen/setup.js'
            ].map(filename => gen[filename](modelConfig, options, config)
        ...
        .then(() => utils.section('Build bundles', () =>
            Promise.all([
                bundleFile(pathResolver('model.js'), {
                    ...jsBundleOptions,
                    noParse: jsBundleOptions.noParse.concat(
                        pathResolver('/gen/model-libs.js')
                    )
                }),
...
function copyCommonFiles(dest, config) {
    utils.section('Copy common files', () => {
        ...
        copyFileIfExists(path.join(discoveryDir, 'dist/lib.umd.js'), dest, 'lib.js');
        copyFileIfExists(path.join(discoveryDir, 'dist/discovery.umd.js'), dest, 'lib.js');
        copyFileIfExists(path.join(discoveryDir, 'dist/lib.css'), dest);
        copyFileIfExists(path.join(discoveryDir, 'dist/discovery.css'), dest, 'lib.css');
    });
}
        ...
        if (config.mode === 'multi') {
            ...
            pipeline = pipeline.then(() =>
                Promise.all([
                    '/index.html',
                    '/gen/setup.js'
                ].map(filename => gen[filename](null, options, config)

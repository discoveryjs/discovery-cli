...
async function convertToSingleFile(content, slug, files, options) {
    ...
    return new ChunkedContent(await content)
        ...
        .replace(/$/, () =>
            streamDataToHtml(getFileContent('model.data'), options.dataCompression)
        );
}
            ...
            for (const modelConfig of models) {
                if (modelConfig.data) {
                    await utils.process(modelConfig.slug, async () => addModelAsset(
                        modelConfig.slug,
                        path.join(modelConfig.slug, 'model.data'),
        ...
        await utils.section('Convert to single file', async () => {
            ...
            for (const [slug, outputPath] of entryPoints) {
                outputFiles.set(outputPath, await utils.section(outputPath, () =>
                    convertToSingleFile(files.get(outputPath), slug, files, options)
                ));
            }
        });
    } else {

...
function generateDataJson(modelConfig, options, cacheDispatcher) {
    ...
    return function getData(req, res) {
        ...
        return cacheDispatcher.get(slug)
            .then(cache => cache
                ? { stream: fs.createReadStream(cache.file), size: cache.size }
...
function dropDataCache(modelConfig, cacheDispatcher) {
    return (req, res) => {
        ...
        cacheDispatcher.get(slug, true);
        routeLog(`/${slug}/`, 'Force cache update');
    ...
    if (cache && ROUTE_DATA in routes) {
        ...
        if (options.warmup) {
            utils.process('Warming up', () => {
                addBeforeReadyTask(`Model '${slug}'`, () => cacheDispatcher.get(slug));
            });
        }
...
module.exports = bootstrap(function createServer(options, config, configFile) {
    const cacheDispatcher = createCacheDispatcher(config.models || [], { configFile, cachedir: options.cache });

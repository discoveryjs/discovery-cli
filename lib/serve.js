...
module.exports = bootstrap(function createServer(options, config, configFile) {
    ...
    app.get('/readyz', (_, res) => {
        if (beforeReadyTasksDone < beforeReadyTasks.length) {
            ...
            res.send({
                ...
                awaitingTime: Date.now() - beforeReadyStartTime
            });
        } else {
            res.status(200);
            res.send({ status: 'OK' });
    ...
    app.listen(options.port, function() {
        ...
        if (beforeReadyTasks.length > 0) {
            console.log(`Await ${beforeReadyTasks.length} tasks before ready (warmup)`);
            ...
            beforeReadyTasks.forEach(task => {
                task.then(() => {
                    ...
                    if (beforeReadyTasksDone === beforeReadyTasks.length) {
                        console.log(`Warmup is DONE in ${((Date.now() - beforeReadyStartTime) / 1000).toFixed(1)} sec`);
                    }
                });
            });
        }

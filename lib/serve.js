...
const defaultRoutes = {
    ...
    [ROUTE_RESET_DATA]: (req, res) => res.status(403).send('Feature is disabled for the model'),
...
function generateArchive(filename, modelConfig, options) {
    return (req, res, next) => {
        gen[filename](modelConfig, options)
            .then(({ stream }) => {
                ...
                const filename = `${slug}-${timestamp}.zip`;
                ...
                return new Promise((resolve, reject) =>
                    stream
                        ...
                        .on('error', reject)
                        .on('finish', () => {
                            utils.logSlugMsg(slug, `Responsed "${modelConfig.download}" as "${filename}" in`, utils.prettyDuration(Date.now() - startTime));
                            resolve();
                        })
                );
            })
...
function bundleAssetRoutes(router, name, config, options, modelConfig) {
...
function createModelRouter(modelConfig, options, config, addBeforeReadyTask, cacheDispatcher, routes = {}) {
    ...
    utils.process('Define default routes', () => {
        ...
        if (options.prebuild) {
            ...
            bundleAssetRoutes(router, 'model', config, options, modelConfig);
        }
    });
...
module.exports = bootstrap(function createServer(options, config, configFile) {
    ...
    if (!config.models || !config.models.length) {
        ...
        const routers = utils.section(
            ...
            () => utils.sortModels(config.models).map(modelConfig =>
                createModelRouter(modelConfig, options, config, addBeforeReadyTask, cacheDispatcher, {
                    ...
                    [ROUTE_RESET_DATA]: options.modelResetCache && dropDataCache(modelConfig, cacheDispatcher),
                    [ROUTE_MODEL_BUILD]: options.modelDownload && generateArchive(ROUTE_MODEL_BUILD, modelConfig, options)
                })
            )
        );
        ...
        if (config.mode === 'single') {
            ...
            app.use('/' + routers[0].slug, ensureTrailingSlash, routers[0]);
        } else {
    ...
    utils.process('Init common routes', () => {
        ...
        bundleAssetRoutes(app, 'index', config, options, null);

...
function createModelRouter(modelConfig, options, config, beforeReady, cacheDispatcher) {
    ...
    utils.process('Define default routes', () => {
        ...
        if (modelConfig.data) {
            ...
            router.get('/data.json', modelDataHandler(modelConfig, options, cacheDispatcher, generateDataEvents));
    ...
    utils.println(`Upload data: ${modelConfig.upload ? 'YES' + (!options.modelDataUpload ? ' (ignored due to options)' : '') : 'NO'}`);
    utils.println(`Embed API: ${modelConfig.embed ? 'YES' : 'NO'}${options.embed !== 'by-config' ? ' (ignored due to --embed option)' : ''}`);
    utils.println(`Download: ${modelConfig.download ? modelConfig.download + (!options.modelDownload ? ' (ignored due to options)' : '') : 'NO'}`);

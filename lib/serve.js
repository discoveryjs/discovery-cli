...
function bundle(router, name, config, options, indexOnly) {
    let incremetalBuild;
    const getAsset = async (type) => {
        let bundle = incremetalBuild
            ? await incremetalBuild.rebuild()
            : incremetalBuild = await makeBundles(config, options, {
                outdir: '/',
                incremental: true
            }, indexOnly ? 'index-only' : 'model-only');

        return bundle.outputFiles.find(file => path.extname(file.path) === '.' + type).text;
    };
...
function createModelRouter(modelConfig, options, config, addBeforeReadyTask, cacheDispatcher, routes = {}) {
    ...
    utils.process('Define default routes', () => {
        ...
        if (options.prebuild) {
            ...
            bundle(router, 'model', { ...config, models: [modelConfig] }, options);
        }
    });
...
module.exports = bootstrap(function createServer(options, config, configFile) {
    ...
    utils.process('Init common routes', () => {
        ...
        bundle(app, 'index', config, options, true);

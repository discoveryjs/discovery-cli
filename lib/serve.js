...
const { getCacheFilename, getCacheFiles } = require('./shared/get-cache-filename');
...
function createModelRouter(modelConfig, options, config, addBeforeReadyTask, addScheduleTask, routes = {}) {
    ...
    const cacheFilename = cache && path.resolve(options.cache, `.discoveryjs.${slug}.[timestamp].cache`);
...
module.exports = bootstrap(function createServer(options, config, configFile) {
    ...
    if (options.cache) {
        addBeforeReadyTask('Schedule obsolete cache clean', () => {
            setInterval(() => {
                if (fs.existsSync(options.cache)) {
                    const cacheFiles = getCacheFiles(options.cache, config.models.map(model => model.slug));
                    for (const caches of Object.values(cacheFiles)) {
                        for (let i = 2; i < caches.length; i++) {
                            fs.unlink(path.resolve(options.cache, caches[i].file), (err) => {
                                utils.println(`Delete obsolete cache: ${caches[i].file}`);
                                if (err) {
                                    console.error(err);
                                }
                            });

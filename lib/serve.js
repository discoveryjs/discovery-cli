...
function createModelRouter(modelConfig, config, options, routes = {}) {
    ...
    utils.process('Define default routes', () => {
        ...
        router.get('/model.js', (req, res) => res.sendFile(path.join(discoveryDir, 'client/model.js')));
        router.get('/model.css', (req, res) => res.sendFile(path.join(discoveryDir, 'client/model.css')));
    });
...
module.exports = bootstrap(function createServer(options, config, configFile) {
    ...
    utils.process('Init common routes', () => {
        app.use(express.static(path.join(discoveryDir, 'client')));

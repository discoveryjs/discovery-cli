...
const { createCacheDispatcher, getCaches } = require('./shared/cache');
const makeBundles = require('./shared/bundle');
...
function bundleAssetRoutes(router, name, config, options, indexOnly) {
    let incremetalBuild;
    let currentBundle;
    let lastBundle;
    const getAsset = async (type) => {
        if (!currentBundle && (!/\.map$/.test(type) || !lastBundle)) {
            if (incremetalBuild) {
                currentBundle = incremetalBuild.rebuild();
            } else {
                currentBundle = makeBundles(config, options, {
                    outdir: '/',
                    // FIXME: Disable incremental build for now, since it's as twice as slower
                    // incremental: true,
                    sourcemap: true
                }, indexOnly ? 'index-only' : 'model-only');
                // currentBundle.then(value => incremetalBuild = value);
            }

            lastBundle = currentBundle;
        }

        const bundle = /\.map$/.test(type) ? lastBundle : currentBundle;

        try {
            return (await bundle).outputFiles.find(file => file.path.endsWith(type)).text;
        } finally {
    ...
    router.get(`/${name}.js`, async (_, res) => {
        res.set('Content-Type', 'application/javascript');
        res.send(await getAsset('.js'));
    });
    router.get(`/${name}.js.map`, async (_, res) => {
        res.set('Content-Type', 'application/json');
        res.send(await getAsset('.js.map'));
    });
    router.get(`/${name}.css`, async (_, res) => {
        res.set('Content-Type', 'text/css');
        res.send(await getAsset('.css'));
    });
    ...
    if (!indexOnly && name !== 'index') {
        router.get(`/${config.models[0].slug}.js.map`, (_, res) => res.redirect(301, `${name}.js.map`));
    }
}
...
function createModelRouter(modelConfig, options, config, addBeforeReadyTask, cacheDispatcher, routes = {}) {
    ...
    utils.process('Define default routes', () => {
        ...
        if (options.prebuild) {
            ...
            bundleAssetRoutes(router, 'model', { ...config, models: [modelConfig] }, options);
        }
    });
...
module.exports = bootstrap(function createServer(options, config, configFile) {
    ...
    const addBeforeReadyTask = (name, fn, critical = false) => beforeReadyTasks.push({
        name: name || 'Untitled',
        status: 'pending',
        critical,
        fn
    ...
    utils.println(`Default darkmode: ${JSON.stringify(config.darkmode) || '<not set>'}`);
    ...
    utils.process('Init common routes', () => {
        ...
        bundleAssetRoutes(app, 'index', config, options, true);
        app.use(express.static(path.join(__dirname, 'static')));
    });

    if (options.cors) {
        console.log('CORS enabled (Access-Control-Allow-Origin: *; Access-Control-Expose-Headers: *)');
    }
    ...
    app.get('/cachez', (_, res) => {
        const caches = getCaches(options.cachedir, config.models || [], true);
        const cacheFiles = [].concat(...Object.values(caches));

        res.status(200);
        res.send({
            cache: options.cache,
            cachedir: options.cachedir && path.relative(process.cwd(), options.cachedir),
            startDate: new Date(beforeReadyStartTime + beforeReadyTimeElapsed),
            models: cacheDispatcher.stat(),
            files: cacheFiles.map(cache => ({
                ...cache,

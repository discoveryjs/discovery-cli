    ...
    return function getData(req, res) {
        ...
        return cacheDispatcher.read(slug)
            ...
            .catch(error => {
                ...
                utils.logSlugError(prefix, `[ERROR] ${String(error).split(/\r\n?|\n/)[0]}`);
            })
            .then(() => {
                utils.logSlugMsg(prefix, '[OK] Responsed in', utils.prettyDuration(Date.now() - startTime));
            });
    };
}
...
function dropDataCache(modelConfig, cacheDispatcher) {
    return (req, res) => {
        ...
        cacheDispatcher.write(slug, true).catch(() => {});

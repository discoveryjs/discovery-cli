...
function bundleAssetRoutes(router, name, config, options, modelConfig) {
    ...
    const getAsset = async (type) => {
        const fn = (modelConfig ? modelConfig.slug : 'index') + type.replace(/\.map$/, '');
        let currentBundle = currentBundles.get(fn);
        ...
        if (!currentBundle || !/\.map$/.test(type)) {
            currentBundle = makeBundle(config, options, {
                ...
                // incremental: true,
                sourcemap: true
            }, entrypoint => path.basename(entrypoint) === fn);

            currentBundles.set(fn, currentBundle);
        }

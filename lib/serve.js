...
const ROUTE_MODEL_BUILD = '/gen/build.html.zip';
const defaultRoutes = {
    ...
    [ROUTE_MODEL_BUILD]: (req, res) => res.send(null)
};
...
function createModelRouter(modelConfig, options, config, addBeforeReadyTask, routes = {}) {
    const { slug } = modelConfig;
    ...
    if (cacheFilename) {
        ...
        if (options.warmup && ROUTE_DATA in routes) {
            utils.process('Warming up', () => {
                addBeforeReadyTask(`Model '${slug}'`, () => routes[ROUTE_DATA](stubApi, stubApi));
            });
        }
...
module.exports = bootstrap(function createServer(options, config, configFile) {
    ...
    const addBeforeReadyTask = (name, fn) => beforeReadyTasks.push({ name, status: 'pending', fn });
    ...
    if (!config.models || !config.models.length) {
        ...
        utils.silent(() =>
            app.use(createModelRouter({ name: 'Discovery' }, options, config, addBeforeReadyTask))
        );
    } else {
        const routers = utils.section(
            config.mode === 'single' ? 'Init single model' : 'Init models',
            () => config.models.map(modelConfig =>
                createModelRouter(modelConfig, options, config, addBeforeReadyTask,
                    options.prebuild
                        ? {
    ...
    app.listen(options.port, function() {
        ...
        if (beforeReadyTasks.length > 0) {
            ...
            beforeReadyTasks.reduce(
                (pipeline, task) => pipeline.then(() => Object.assign(task, {
                    status: 'processing',
                    startTime: Date.now()
                }).fn())
                    .catch(error => console.error(`Warmup task "${task.name}" error:`, task.error = error))

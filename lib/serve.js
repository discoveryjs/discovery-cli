...
function createModelRouter(modelConfig, options, config, addBeforeReadyTask, addScheduleTask, routes = {}) {
    const { slug } = modelConfig;
    const cacheFilename = getCacheFilename(modelConfig, true);
        ...
        addBeforeReadyTask('Schedule obsolete cache clean', () => {
            setInterval(() => {
                if (fs.existsSync(options.cache)) {
                    const cacheFiles = getCacheFiles(options.cache);
                    for (const caches of Object.values(cacheFiles)) {
                        const cachesSorted = caches.sort((a, b) => b.timestamp - a.timestamp);
                        for (let i = 2; i < cachesSorted.length; i++) {
                            fs.unlink(path.resolve(options.cache, cachesSorted[i].file), () => {});
                        }
                    }
                }
            }, 300000);

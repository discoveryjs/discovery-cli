...
const types = {
    ...
    'css': (modelConfig, baseURI) =>
        toConcatContent(getAssetList(modelConfig.view || {}, baseURI, '.css')),
    'libs-js': (modelConfig, baseURI, options) =>
        toJsLib(getLibsList(modelConfig.view || {}, baseURI, '.js'), options.es5LibsJs),
    'libs-css': (modelConfig, baseURI) =>
        toCSSLib(getLibsList(modelConfig.view || {}, baseURI, '.css'))
};
...
function toJsLib(libs, es5) {
    return [
        ...
        ...libs.reduce((lines, { name, files }) =>
            lines.concat(
                '',
                `${es5 ? 'exports.' : 'export const ' }${name} = (function(){`,
                'var exports = {};',

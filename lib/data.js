...
function getData(modelConfig, { rewriteCache, warmup, pretty } = {}) {
    const cacheFile = getCacheFilename(modelConfig, rewriteCache);
    ...
    return collectData(modelConfig).then(data => {
        ...
        if (cacheFile) {
            const tmpCacheFile = ensureDir(path.join(os.tmpdir(), path.basename(cacheFile)));
            const startWriteTime = Date.now();
            const writeToFileStream = jsonStream
                .pipe(fs.createWriteStream(tmpCacheFile))
                .on('finish', () => {
                    fs.renameSync(tmpCacheFile, ensureDir(cacheFile));

...
module.exports = bootstrap.model(function getCache(modelConfig, { mode, pretty, hash }) {
    const { slug } = modelConfig;
    const actualCache = mode !== 'force'
        ? getActualCache(modelConfig, hash)
        : false;
    ...
    const newCacheFilename = genCacheFilename(modelConfig, hash);
    ...
    return genData.fn(modelConfig).then(data => new Promise((resolve, reject) => {
        ...
        stringifyStream(data, null, pretty)
            ...
            .on('finish', () => {
                ...
                resolve(genCacheInfo(newCacheFilename));
            });
    }));
});
